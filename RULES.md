...
- **Model Structure**:
  ...
  - **Aliasing**: At least one representative for each unique physical quantity (alias set) **must** be listed in the corresponding section.
  - **Completeness**: All output variables and state derivatives **must** be listed (using at least one variable from each alias set).
  - **InitialUnknowns**: **Must** include all unpinned outputs, calculated parameters, and unpinned states/derivatives.
  - **Pinning**: Variables that are "pinned" (e.g., via `initial="exact"`, or derivatives whose corresponding states are `initial="exact"`) are optional in `InitialUnknowns`.
...
- **Model Structure**:
  ...
  - **Aliasing**: At least one representative for each unique physical quantity (alias set) **must** be listed in the corresponding section.
  - **Mandatory InitialUnknowns**: **Must** be provided for non-clocked outputs, calculated parameters, and states/derivatives with `initial="approx"` or `"calculated"` that are not pinned by initial conditions (e.g., via `initial="exact"` or equivalent).
