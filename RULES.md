...
- **Zip Slip (Path Traversal)**: Normalized paths **must not** escape the archive root (e.g., `foo/../../etc/passwd`).
...
- **Central Directory vs Local Header Mismatch**: Metadata (filenames, sizes, offsets) **must** be consistent between the central directory and local file headers.
- **Overlapping File Entries**: No two local file data regions (headers + compressed data) **must** overlap in the archive.
...
- **Extra Field and Comment Integrity**:
...
- **Local File Header Signatures**: The local file header signature (`PK\x03\x04`) **must** be present at the reported offset for every central directory entry.
- **Total Entry Count Sanity**: The total number of entries reported in the central directory **must** match the actual count of local file records found.
