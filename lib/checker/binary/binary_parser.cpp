...
static std::set<std::string> parsePe(std::ifstream& f)
{
    ...
    if (!readFromFile(f, 0, dos) || dos.e_magic != DOS_MAGIC)
        return {};

    uint32_t pe_sig = 0;
    if (!readFromFile(f, static_cast<std::streamoff>(dos.e_lfanew), pe_sig) || pe_sig != PE_MAGIC)
        return {};

    IMAGE_FILE_HEADER file_hdr{};
    if (!readFromFile(f, static_cast<std::streamoff>(dos.e_lfanew + 4), file_hdr))
        return {};

    uint32_t optional_hdr_off = dos.e_lfanew + 4 + sizeof(IMAGE_FILE_HEADER);
    uint16_t magic = 0;
    if (!readFromFile(f, static_cast<std::streamoff>(optional_hdr_off), magic))
        return {};
    ...
    if (magic == PE32_MAGIC)
    {
        ...
        if (readFromFile(f, static_cast<std::streamoff>(optional_hdr_off), opt))
        {
    ...
    if (!readFromFile(f, static_cast<std::streamoff>(export_dir_off), export_dir))
        return {};
...
std::set<std::string> BinaryParser::getExports(const std::filesystem::path& path)
{
    ...
    else if (magic == FAT_MAGIC || magic == FAT_CIGAM) // Fat Binary
    {
        fat_header fh{};
        readFromFile(f, 0, fh);
        uint32_t nfat = (magic == FAT_CIGAM) ? ((fh.nfat_arch << 24) | ((fh.nfat_arch << 8) & 0xFF0000) |
        ...
        for (uint32_t i = 0; i < nfat; ++i)
        {
            ...
            uint32_t offset = (magic == FAT_CIGAM) ? ((fa.offset << 24) | ((fa.offset << 8) & 0xFF0000) |
