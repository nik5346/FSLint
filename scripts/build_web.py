...
Usage:
    ...
    python scripts/build_web.py --clean
    python scripts/build_web.py --skip-web
"""
...
def em_env() -> dict:
    """
    Build an environment dict with the EMSDK vars that emcmake/emmake need.
    """
    env = os.environ.copy()
    if EMSDK_ROOT:
        if os.name == 'nt':
...
def find_ninja() -> Path:
    """
    Try to find the Ninja executable.
    """
    # First, try to find ninja in PATH
    try:
        ninja_cmd = "where ninja" if os.name == 'nt' else "which ninja"
        result = subprocess.run(ninja_cmd, shell=True, capture_output=True, text=True)
...
def emsdk_script() -> Path:
    return EMSDK_ROOT / ("emsdk.bat" if os.name == 'nt' else "emsdk")

def emcmake() -> Path:
    if os.name == 'nt':
        return EMSDK_ROOT / "upstream" / "emscripten" / "emcmake.bat"
    return EMSDK_ROOT / "upstream" / "emscripten" / "emcmake"

def emmake() -> Path:
    if os.name == 'nt':
        return EMSDK_ROOT / "upstream" / "emscripten" / "emmake.bat"
    return EMSDK_ROOT / "upstream" / "emscripten" / "emmake"
...
def find_or_install_emsdk():
    ...
    run(emsdk_script(), "install", "latest", cwd=EMSDK_ROOT)
    run(emsdk_script(), "activate", "latest", cwd=EMSDK_ROOT)
    if not tools_present():
        print("[ERROR] Installation finished but emcmake still not found.")
        sys.exit(1)
...
def download_emsdk(target: Path):
    ...
    if os.name != 'nt':
        ...
        emsdk_bin = target / "emsdk"
        if emsdk_bin.exists():
            emsdk_bin.chmod(0o755)
...
def build_wasm():
    ...
    if not BUILD_DIR.exists():
        ...
        run(*cmake_args, cwd=BUILD_DIR, use_em_env=True)
    else:
...
def copy_artifacts():
    ...
    for asset in [ROOT / "images" / "icon.svg", ROOT / "images" / "banner.svg", ROOT / "RULES.md", ROOT / "LICENSE"]:
        if asset.exists():
            dst = web_public_root / asset.name.lower()
            shutil.copy(asset, dst)
            print(f"  {asset.name} -> {dst}")
